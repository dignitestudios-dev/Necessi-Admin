    ...
    <div className="p-6 h-full w-full overflow-auto">
      ...
      {selectedTransaction && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-gray-50 p-6 rounded-lg shadow-lg max-w-lg w-full">
            <h2 className="text-2xl font-semibold text-black mb-4">Transaction Details</h2>
            <div className="mb-4 text-gray-800">
              <p className="text-md font-medium">User: {selectedTransaction.user}</p>
              <p className="text-md font-medium">Transaction Date: {selectedTransaction.transactionDate}</p>
              <p className="text-md font-medium">Amount: {selectedTransaction.totalAmount}</p>
              <p className="text-md font-medium">Platform Fee: {selectedTransaction.platformFee}</p>
              <p className="text-md font-medium">Status: {selectedTransaction.status}</p>
              {/* <p className="text-md font-medium">Source: {selectedTransaction.source}</p> */}
              <p className="text-md font-medium">Total Revenue: ${calculateTotalRevenue(selectedTransaction).toLocaleString()}</p>
              <p className="text-md font-medium">Platform Earnings: ${calculatePlatformEarnings(selectedTransaction).toLocaleString()}</p>
            </div>
            <button
              onClick={closeModal}
              className="bg-[#074F57] text-white py-2 px-4 rounded-lg mt-4 w-full"
            >
              Close
            </button>
          </div>
